...
## 分析要件
1. **メール種別の判定**: このメールが営業メール（売り込み）か、それとも印刷サービスに関する genuine な問い合わせかを正確に判定してください
...
## 判断基準
- 営業メールの特徴：挨拶が定型、自社サービスの宣伝、相手のニーズが不明確
- 印刷問い合わせの特徴：具体的な仕様、サイズ、部数、納期などの記載がある
- その他：問い合わせ内容が不明確または印刷と無関係

重要：営業メールと判断した場合は「対応不要」と明記してください。`;
...
2.  **本物のリードの場合**:
    ...
    c. **見積案作成**: 問い合わせ内容から顧客の具体的な要望を正確に読み取り、それに基づき現実的で詳細な見積の明細項目を作成してください。特に以下の点を考慮してください：
       - サイズ、綴じ方向、ページ数、カラー設定、用紙の種類、部数などの具体的な仕様を反映
       - 顧客が希望する仕様に基づいた適切な単価と数量を設定
       - undefinedや不適切な項目名を避け、具体的な品名を使用
