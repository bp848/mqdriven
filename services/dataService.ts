...
export const getApplications = async (currentUser: User | null): Promise<ApplicationWithDetails[]> => {
    ...
    const draftsPromise = (async () => {
        const withApplicantQuery = supabase
        ...
        const result = await withApplicantQuery;
        if (result.error && /relationship/i.test(result.error.message || '')) {
            console.warn('[getApplications] Falling back to draft query without applicant join:', result.error.message);
