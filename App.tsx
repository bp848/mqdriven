...
const App: React.FC = () => {
    ...
    return (
        <div className="h-screen overflow-hidden bg-slate-100 dark:bg-slate-900 text-slate-800 dark:text-slate-200 font-sans flex flex-col mq-theme">
            <Sidebar
            ...
            />
            <main className="flex-1 flex flex-col overflow-hidden bg-slate-100 dark:bg-slate-900 relative min-h-0 pt-16 sm:pt-16">
                {dbError && <GlobalErrorBanner error={dbError} onRetry={loadAllData} onShowSetup={() => setIsSetupModalOpen(true)} />}
                <div className={`flex-1 overflow-y-auto px-4 sm:px-6 pb-6 bg-slate-100 dark:bg-slate-900 transition-opacity duration-150 ${isLoading && !dbError ? 'opacity-50 pointer-events-none' : ''}`}>
                    <Header {...headerConfig} />
                    <div className="mt-6">
                        <PageShell>
                            {renderContent()}
