...
const CustomerInfoForm: React.FC<CustomerInfoFormProps> = ({ customerId }) => {
    ...
    const inputClass = 'block w-full rounded-md border border-slate-300 bg-white px-3 py-2 text-sm text-slate-900 shadow-sm focus:border-blue-500 focus:ring-blue-500 disabled:cursor-not-allowed disabled:bg-slate-100';
    ...
    if (!hasLoaded && isLoading) {
        return (
            <div className="flex items-center gap-3 rounded-lg border border-slate-200 bg-white/70 p-4 text-slate-600">
    ...
    if (!hasLoaded && loadError) {
        return (
            <div className="flex flex-wrap items-center gap-3 rounded-lg border border-red-200 bg-red-50 p-4 text-sm text-red-700">
                ...
                <button type="button" onClick={handleRetry} className="rounded-md border border-red-200 px-3 py-1 text-xs font-semibold text-red-700 hover:bg-red-100">
                    再試行
                </button>
            </div>
        );
    ...
    return (
        <form onSubmit={handleSubmit} className="space-y-6">
            {status && (
                <div className={`flex items-center gap-3 rounded-lg border p-4 text-sm ${status.type === 'success' ? 'border-green-200 bg-green-50 text-green-700' : 'border-red-200 bg-red-50 text-red-700'}`}>
                    {status.type === 'success' ? <CheckCircle className="h-5 w-5" /> : <AlertTriangle className="h-5 w-5" />}
                    {status.message}
                </div>
            )}

            <div className="grid grid-cols-1 gap-4 rounded-lg border border-slate-200 bg-slate-50 p-4 text-sm text-slate-600 md:grid-cols-3">
                <div>
                    <p className="text-xs font-semibold uppercase tracking-wide text-slate-500">顧客ID</p>
                    <p className="mt-1 text-base font-medium text-slate-900">{info.id}</p>
                </div>
                <div>
                    <p className="text-xs font-semibold uppercase tracking-wide text-slate-500">作成日時</p>
                    <p className="mt-1 text-base text-slate-900">{formatDateTime(info.createdAt)}</p>
                </div>
                <div>
                    <p className="text-xs font-semibold uppercase tracking-wide text-slate-500">最終更新</p>
                    <p className="mt-1 text-base text-slate-900">{formatDateTime(info.updatedAt)}</p>
                </div>
            </div>

            {SECTIONS.map(section => (
                <section key={section.key} className="space-y-4 rounded-2xl border border-slate-100 bg-white p-4 shadow-sm">
                    <div>
                    ...
                    <div className={`grid grid-cols-1 gap-4 ${section.key === 'karte' ? '' : 'md:grid-cols-2'}`}>
                        {section.fields.map(field => (
                            <div key={`${section.key}_${String(field.key)}`} className="flex flex-col gap-1">
                                <label className="text-sm font-medium text-slate-700" htmlFor={`${section.key}_${String(field.key)}`}>
                                    {field.label}
                                </label>
                                {field.type === 'textarea' ? (
                                    <textarea
            ...
            <div className="flex justify-end">
                <button
                    type="submit"
                    disabled={isSaving || !hasLoaded}
                    className="inline-flex items-center gap-2 rounded-lg bg-blue-600 px-5 py-2 text-white shadow-sm transition hover:bg-blue-700 disabled:cursor-not-allowed disabled:bg-slate-400"
                >
