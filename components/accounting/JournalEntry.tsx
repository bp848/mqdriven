...
export const JournalReviewPage: React.FC<JournalReviewPageProps> = ({ notify }) => {
  ...
  const loadDrafts = useCallback(async () => {
    setIsLoading(true);
    try {
      const response = await fetch('/api/accounting/journal-drafts');
      if (!response.ok) {
        const errorData = await response.json();
        throw new Error(errorData.details || 'Server responded with an error');
      }
      const fetchedDrafts: DraftJournalEntry[] = await response.json();
      ...
      if (notify) notify(`仕訳下書きの読み込みに失敗しました: ${error instanceof Error ? error.message : 'Unknown error'}`, 'error');
    } finally {
      setIsLoading(false);
    }
  }, [notify]);
  ...
  const handleApprove = async (batchId: string) => {
    if (!notify) return;
    try {
      const response = await fetch(`/api/accounting/journal-batches/${batchId}/approve`, {
        method: 'POST',
        headers: {
      ...
      notify(`仕訳バッチID: ${batchId.substring(0,8)}... を承認しました`, 'success');
      await loadDrafts(); // Refresh the list
    } catch (error) {
        console.error('Failed to approve journal batch:', error);
        notify(`仕訳の承認に失敗しました: ${error instanceof Error ? error.message : 'Unknown error'}`, 'error');
    }
  };

  const handleBatchApprove = () => {
      // This function needs to be adapted for real data
      if (notify) notify('一括承認は現在実装されていません。', 'info');
  };

  const handleGenerateAI = () => {
      // This function needs to be adapted for real data
      if (notify) notify('AI生成テストは現在実装されていません。', 'info');
  };
                   ...
                   drafts.map(draft => (
                     ...
                     >
                        <div className="flex justify-between items-start mb-1">
                           ...
                           {draft.status === 'draft' && (draft.confidence ?? 1) < 0.8 && <AlertTriangle className="w-4 h-4 text-amber-500" />}
                        </div>
                        ...
                        {draft.status === 'draft' && (
                            <div className="mt-2 h-1 w-full bg-slate-100 rounded-full overflow-hidden">
                                <div 
                                    className={`h-full rounded-full ${((draft.confidence ?? 1) > 0.9) ? 'bg-emerald-400' : ((draft.confidence ?? 1) > 0.7) ? 'bg-amber-400' : 'bg-red-400'}`} 
                                    style={{ width: `${(draft.confidence ?? 1) * 100}%` }}
                                ></div>
                            </div>
                        )}
                     </div>
            ...
            {selectedDraft ? (
                <>
                   ...
                   <div className={`p-4 flex items-center gap-3 ${
                       (selectedDraft.confidence ?? 1) < 0.8 ? 'bg-amber-50 border-b border-amber-100' : 
                       'bg-indigo-50/30 border-b border-slate-100'
                   }`}>
                       {(selectedDraft.confidence ?? 1) < 0.8 ? (
                           <>
                           ...
                           <>
                             ...
                             <div className="flex-1">
                                ...
                                <p className="text-xs text-indigo-600">システムルールに基づき{Math.round((selectedDraft.confidence ?? 1) * 100)}%の確率で推論されました。</p>
                             </div>
                           </>
                       )}
                   </div>
                   ...
                   <div className="p-6 border-t border-slate-200 bg-slate-50 flex justify-center gap-4">
                       ...
                       >
                           {selectedDraft.status === 'posted' ? (
                               <>
                                <CheckCircle className="w-5 h-5" /> 承認済み (Posted)
                               </>
                           ) : (
                               <>
