...
const setupSteps: Array<{ title: string; description: string; code?: string; }> = [
    ...
    },
    {
        ...
        description: `SQL Editorで次のクエリを実行し、承認ルートに登録したいユーザー（通常は自分）のIDを控えてください。既定では ${ADMIN_USER_ID} を埋め込んでいます。`,
        code: 'select id, email from auth.users order by created_at desc;',
    },
    {
        title: '以下のスクリプトを貼り付けて実行する',
        description: 'スクリプト全体をコピーし、ステップ2で控えたUUIDに必ず差し替えてから実行してください。何度実行しても安全なように作られているため、失敗した場合は再実行できます。',
    },
    {
