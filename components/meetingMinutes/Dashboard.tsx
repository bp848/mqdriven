...
export const MeetingDashboard: React.FC<MeetingDashboardProps> = ({ meetings, onNewMeeting, onSelectMeeting }) => {
  ...
  const filteredAndSortedMeetings = useMemo(() => {
    ...
    if (searchTerm.trim()) {
      ...
      result = result.filter((meeting) =>
        meeting.title.toLowerCase().includes(lowerTerm) ||
        meeting.summary.toLowerCase().includes(lowerTerm) ||
        meeting.transcript.toLowerCase().includes(lowerTerm),
      );
    }
      ...
      <div className="space-y-4">
        ...
        {filteredAndSortedMeetings.length === 0 ? (
          <div className="bg-slate-900/50 border border-slate-800 rounded-xl p-12 text-center">
            ...
            {!searchTerm && (
              <button onClick={onNewMeeting} className="text-brand-400 hover:text-brand-300 font-medium">
                最初の会議を始める &rarr;
              </button>
            )}
          </div>
              ...
              >
                ...
                <div className="flex items-center justify-between text-xs text-slate-500 pt-4 border-t border-slate-800">
                  ...
                  <div className="flex items-center gap-2">
                    <span className="text-slate-600">{new Date(meeting.date).toLocaleDateString('ja-JP')}</span>
