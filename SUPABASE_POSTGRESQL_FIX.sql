-- 顧客別案件予算見える化ビュー作成SQL（Supabase/PostgreSQL専用版）
-- SupabaseのPostgreSQL環境に最適化
...
BEGIN
    ...
    RAISE NOTICE '📊 Supabaseテーブル確認: projects=%, customers=%, orders=%', projects_count, customers_count, orders_count;
...
SELECT 
    ...
    -- 利益率の計算（PostgreSQLのNULLIF使用）
    ROUND(
        NULLIF(
            (COALESCE(SUM(p.amount), 0) - COALESCE(SUM(p.total_cost), 0)) * 100.0 / 
            NULLIF(COALESCE(SUM(p.amount), 0), 0), 0
        ), 2
    ) as profit_margin,
...
-- ビュー作成の成功確認（Supabase対応）
DO $$
BEGIN
    RAISE NOTICE '✅ Supabase customer_budget_summary_viewが正常に作成されました。';
    ...
    RAISE NOTICE '🚀 Supabase最適化: UUID対応、LATERAL JOIN使用';
EXCEPTION WHEN OTHERS THEN
    RAISE NOTICE '❌ Supabaseビュー作成エラー: %', SQLERRM;
