...
console.info("google-oauth-disconnect ready");
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (!isJson) {
      return jsonResponse(req, { error: "content-type must be application/json" }, 400);
    }
    ...
    const resp = await fetch(`${supabaseUrl}/rest/v1/user_google_tokens?user_id=eq.${userId}`, {
      method: "DELETE",
      headers: {
        apikey: serviceRole,
    ...
    if (!resp.ok) {
      ...
      console.error("google-oauth-disconnect delete failed", resp.status, text);
      return jsonResponse(req, { error: "failed to disconnect" }, 500);
    }
