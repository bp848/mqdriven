...
cleaned as (
    select
        ...
        case
            when regexp_replace(trim(e.tax_rate::text), '[^0-9\\.-]', '', 'g') ~ '^[+-]?[0-9]+(\\.[0-9]+)?$'
                then regexp_replace(trim(e.tax_rate::text), '[^0-9\\.-]', '', 'g')::numeric
        end as tax_rate_num,
        case
            when regexp_replace(trim(e.consumption::text), '[^0-9\\.-]', '', 'g') ~ '^[+-]?[0-9]+(\\.[0-9]+)?$'
                then regexp_replace(trim(e.consumption::text), '[^0-9\\.-]', '', 'g')::numeric
        end as consumption_num,
        case
            when regexp_replace(trim(e.variable_cost::text), '[^0-9\\.-]', '', 'g') ~ '^[+-]?[0-9]+(\\.[0-9]+)?$'
                then regexp_replace(trim(e.variable_cost::text), '[^0-9\\.-]', '', 'g')::numeric
    ...
    left join public.projects p on coalesce(
        nullif(trim(e.project_id::text), ''),
        nullif(trim(to_jsonb(e)->>'projectid'), ''),
        nullif(trim(to_jsonb(e)->>'project_code'), '')
    ) = coalesce(
        nullif(trim(p.project_id::text), ''),
        nullif(trim(p.id::text), ''),
        nullif(trim(p.project_code::text), '')
    )
...
mq as (
    select
        ...
        ) as project_name_resolved,
        coalesce(
            nullif(calc.customer_name_joined::text, ''),
            nullif(calc.customer_name::text, ''),
            nullif(calc.pattern_name::text, '')
        ) as customer_name_resolved,
        coalesce(
            nullif(
                concat_ws(
                    '｜',
...
with cleaned as (
    select
        ed.*,
        coalesce(
            nullif(trim(to_jsonb(ed)->>'item_name'), ''),
            nullif(trim(to_jsonb(ed)->>'item'), ''),
            nullif(trim(to_jsonb(ed)->>'product_name'), ''),
            nullif(trim(to_jsonb(ed)->>'name'), '')
        ) as item_name_clean,
        case
            when regexp_replace(trim(ed.quantity::text), '[^0-9\\.-]', '', 'g') ~ '^[+-]?[0-9]+(\\.[0-9]+)?$'
                then regexp_replace(trim(ed.quantity::text), '[^0-9\\.-]', '', 'g')::numeric
        ...
        case
            when regexp_replace(trim(ed.unit_price::text), '[^0-9\\.-]', '', 'g') ~ '^[+-]?[0-9]+(\\.[0-9]+)?$'
                then regexp_replace(trim(ed.unit_price::text), '[^0-9\\.-]', '', 'g')::numeric
        end as unit_price_num,
        case
            when regexp_replace(trim(ed.variable_cost::text), '[^0-9\\.-]', '', 'g') ~ '^[+-]?[0-9]+(\\.[0-9]+)?$'
                then regexp_replace(trim(ed.variable_cost::text), '[^0-9\\.-]', '', 'g')::numeric
